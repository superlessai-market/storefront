...
### AdSense 심사용 정보 섹션 (앱 본체와 푸터 사이에 삽입)

AdSense가 앱만 있는 페이지를 "콘텐츠 부족(thin content)"으로 거절하므로, 텍스트 콘텐츠가 필요합니다.
앱 본체 아래, 푸터 위에 `.info-section`을 추가하세요.

```html
<!-- Info section for SEO / AdSense -->
<section class="info-section">
  <!-- APP: 앱 소개 (항상 보임) — 앱이 뭔지, 어떤 기술을 쓰는지 2~3문장 -->
  <h2>앱이름이란?</h2>
  <p>앱 소개 문구. 어떤 기술을 사용하고, 어떤 결과를 보여주는지 설명.</p>

  <!-- APP: 사용법 (접혀 있음) — 3단계 ol 리스트 -->
  <details>
    <summary>사용법</summary>
    <ol>
      <li><strong>1단계</strong> — 설명</li>
      <li><strong>2단계</strong> — 설명</li>
      <li><strong>3단계</strong> — 설명</li>
    </ol>
  </details>

  <!-- APP: FAQ (접혀 있음) — 개인정보, 정확도, 무료 여부, 브라우저 호환성 -->
  <details>
    <summary>자주 묻는 질문</summary>
    <dl>
      <dt>개인정보가 저장되나요?</dt>
      <dd>아닙니다. 모든 처리는 브라우저에서 이루어지며 서버로 전송되지 않습니다.</dd>

      <dt>결과는 정확한가요?</dt>
      <dd>앱별 면책 답변.</dd>

      <dt>무료인가요?</dt>
      <dd>네, 완전 무료입니다. 회원가입도 필요 없습니다.</dd>

      <dt>어떤 브라우저에서 사용할 수 있나요?</dt>
      <dd>Chrome, Safari, Firefox, Edge 등 최신 브라우저에서 모두 사용 가능합니다.</dd>
    </dl>
  </details>
</section>
```
...
규칙:
- **순수 HTML, JS 의존 없음** — 크롤러가 `<details>` 안 텍스트를 전부 인덱싱함
- **앱 소개** (`<h2>` + `<p>`) — 항상 보임
- **사용법** (`<details><summary>`) — 접혀 있음
- **자주 묻는 질문** (`<details><summary>`) — 접혀 있음
- 스타일은 앱 디자인에 맞게 조정하되, 흰색 카드 형태 권장 (`.info-section`)
...
- 앱 아래, 푸터 위에 `.info-section` 흰색 카드 추가
- 앱 소개 (`<h2>` + `<p>`) — 항상 보임. 2~3문장.
- 사용법 (`<details><summary>`) — 접혀 있음. 3단계 `<ol>`.
- 자주 묻는 질문 (`<details><summary>`) — 접혀 있음.
- 순수 HTML, JS 의존 없음
```
...
[ ] 5. index.html에 AdSense 스크립트 + 정보 섹션(.info-section) + 푸터 추가
[ ] 6. thumbnail.png 추가
[ ] 7. wrangler.jsonc 추가
[ ] 8. LICENSE 추가
[ ] 9. CLAUDE.md 작성
